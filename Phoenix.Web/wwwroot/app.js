...
async function refreshSignals() {
  try {
    ...
    document.querySelector('#orders').innerHTML = active.length ? active.map(s => `
      ...
      <div class="similarity ${s.targetSimilarityPercent == null ? 'insufficient' : ''}">${s.targetSimilarityPercent == null ? `برای پیش‌بینی، دادهٔ فنی کافی نیست (${fa.format(s.similaritySampleCount || 0)} نتیجه)` : `<div class="similarity-meter target-similarity"><span>احتمال رسیدن به تارگت <b>${fa.format(s.targetSimilarityPercent ?? 0)}٪</b></span><i><em style="width:${Math.max(0,Math.min(100,s.targetSimilarityPercent ?? 0))}%"></em></i></div><div class="similarity-meter stop-similarity"><span>احتمال رسیدن به استاپ <b>${fa.format(s.stopSimilarityPercent ?? 0)}٪</b></span><i><em style="width:${Math.max(0,Math.min(100,s.stopSimilarityPercent ?? 0))}%"></em></i></div><small>${fa.format(s.similaritySampleCount)} نتیجهٔ فنی آموخته‌شده</small>`}</div>
