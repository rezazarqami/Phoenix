...
async function refreshSignals() {
  try {
    ...
    document.querySelector('#orders').innerHTML = active.length ? active.map(s => `
      ...
      <div class="similarity ${s.targetSimilarityPercent == null && s.stopSimilarityPercent == null ? 'insufficient' : ''}">${s.targetSimilarityPercent == null && s.stopSimilarityPercent == null ? `برای محاسبه شباهت، سابقه نتیجه‌دار کافی نیست (${fa.format(s.similaritySampleCount || 0)} نمونه)` : `<div class="similarity-meter target-similarity"><span>شباهت مستقل به تارگت <b>${fa.format(s.targetSimilarityPercent ?? 0)} از ۱۰۰</b></span><i><em style="width:${Math.max(0,Math.min(100,s.targetSimilarityPercent ?? 0))}%"></em></i></div><div class="similarity-meter stop-similarity"><span>شباهت مستقل به استاپ <b>${fa.format(s.stopSimilarityPercent ?? 0)} از ۱۰۰</b></span><i><em style="width:${Math.max(0,Math.min(100,s.stopSimilarityPercent ?? 0))}%"></em></i></div><small>${fa.format(s.similaritySampleCount)} الگوی فنی آموخته‌شده</small>`}</div>
